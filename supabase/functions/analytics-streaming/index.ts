...
async function handleAnalyticsStreaming(req: Request, logger: SecurityLogger): Promise<Response> {
  try {
    ...
    logger.error('Analytics streaming error', error as Error);
    return new Response(
      JSON.stringify({ 
        error: error.message,
        success: false
...
export default withSecurity(handleAnalyticsStreaming, {
  requireAuth: true,
