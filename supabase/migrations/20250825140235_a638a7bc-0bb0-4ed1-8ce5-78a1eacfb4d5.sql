-- Complete RLS Recursion Fix - Handle Dependencies Properly
-- Phase 1: Drop all dependent policies first

-- Drop all team_members policies that might depend on functions
...
-- Phase 2: Now drop the problematic functions
...
DROP FUNCTION IF EXISTS public.is_team_member_safe(uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.is_team_owner_safe(uuid, uuid) CASCADE;

-- Phase 3: Create truly safe security definer functions that disable RLS
...
-- Phase 4: Create simple, direct RLS policies without recursion
