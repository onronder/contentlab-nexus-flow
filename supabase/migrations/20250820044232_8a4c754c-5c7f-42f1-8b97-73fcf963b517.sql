-- Phase 2A: Fix remaining function security warnings only first
...
-- 2. Create basic production monitoring tables without foreign key constraints first
...
-- Session recordings policies (simplified for now)
CREATE POLICY "System can manage session recordings" ON public.session_recordings
  FOR ALL USING (true) WITH CHECK (true);
