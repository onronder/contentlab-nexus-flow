-- Force refresh and ensure proper view security
-- This migration ensures the view is properly configured without security definer

-- Drop and recreate with explicit security settings
DROP VIEW IF EXISTS public.competitor_details CASCADE;

-- Recreate as a standard view that relies on RLS
CREATE VIEW public.competitor_details 
SECURITY INVOKER  -- Explicitly set to use invoker's privileges (not definer)
AS
SELECT 
    pc.id,
    pc.project_id,
...
-- Verify the view uses invoker privileges
COMMENT ON VIEW public.competitor_details IS 'Competitor details view with SECURITY INVOKER - uses caller permissions and respects RLS policies on underlying tables.';