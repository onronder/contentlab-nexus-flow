...
CREATE TABLE IF NOT EXISTS public.behavioral_analytics (
  ...
  INDEX (behavior_type, created_at),
...
CREATE INDEX idx_rate_limit_usage_identifier ON public.rate_limit_usage(identifier, endpoint);
CREATE INDEX idx_security_incidents_user_id ON public.security_incidents(user_id, created_at DESC);
CREATE INDEX idx_user_security_locks_user_id ON public.user_security_locks(user_id, is_active);
CREATE INDEX idx_ip_blocklist_ip ON public.ip_blocklist(ip_address, is_active);
CREATE INDEX idx_security_alerts_user_id ON public.security_alerts(user_id, created_at DESC);
CREATE INDEX idx_behavioral_analytics_user_session ON public.behavioral_analytics(user_id, session_id, created_at);
CREATE INDEX idx_threat_intelligence_indicator ON public.threat_intelligence(indicator_type, indicator_value, is_active);