-- Insert test data for content management system (avoiding duplicates)

-- First, get the existing category IDs and use them, or create new ones with different names
DO $$
DECLARE
...
BEGIN
    ...
    SELECT id INTO marketing_cat_id FROM public.content_categories WHERE name = 'Marketing' LIMIT 1;
    SELECT id INTO docs_cat_id FROM public.content_categories WHERE name = 'Documentation' LIMIT 1;
    SELECT id INTO blog_cat_id FROM public.content_categories WHERE name = 'Blog Posts' LIMIT 1;
    SELECT id INTO video_cat_id FROM public.content_categories WHERE name = 'Videos' LIMIT 1;
    SELECT id INTO image_cat_id FROM public.content_categories WHERE name = 'Images' LIMIT 1;
    
    -- Insert test content items using existing categories
