-- Phase 2: Async Job Processing System (Fixed)
-- Create enhanced job queue system for AI operations
...
CREATE POLICY "System can manage all AI jobs"
...
CREATE POLICY "Team admins can view security events"
  ...
  USING (
    ...
    EXISTS (
      ...
      AND tm.team_id = security_events.team_id
...
CREATE POLICY "Team managers can view team AI usage"
  ON public.ai_usage_analytics FOR SELECT
  USING (
    team_id IN (
      SELECT get_user_teams_safe.team_id FROM get_user_teams_safe(auth.uid())
    ...
    EXISTS (
      ...
      JOIN user_roles ur ON tm.role_id = ur.id
      WHERE tm.user_id = auth.uid() 
      AND tm.team_id = ai_usage_analytics.team_id
      AND tm.is_active = true 
      AND tm.status = 'active'
...
CREATE POLICY "System can manage AI usage analytics"
