-- Fix infinite recursion in team_members RLS policies and member_status enum issue

-- First, ensure member_status enum exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'member_status') THEN
        CREATE TYPE public.member_status AS ENUM ('pending', 'active', 'inactive', 'left', 'removed');
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can view members of accessible teams" ON public.team_members;
...
CREATE POLICY "Users can insert team memberships for teams they own"
ON public.team_members FOR INSERT
WITH CHECK (public.is_team_owner_safe(team_id, auth.uid()));

-- Drop and recreate teams RLS policies to use security definer functions
...
USING (
  ...
  id IN (SELECT team_id FROM public.team_members WHERE user_id = auth.uid() AND is_active = true AND status = 'active'::member_status)
);
...
-- Fix the create_team_with_member_integration function to properly handle member_status enum
...
BEGIN
  ...
  -- Add owner as team member with explicit casting
  INSERT INTO public.team_members (
    ...
    new_team_id,
    owner_role_id,
    'active'::public.member_status,  -- Explicitly cast to the enum type
    true,
    now(),
    current_user_id
  );
