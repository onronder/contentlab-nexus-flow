-- Phase 2A: Fix remaining function security warnings and create production tables
...
-- 2. Move remaining extensions out of public schema
DO $$
BEGIN
  -- Create extensions schema if it doesn't exist
  CREATE SCHEMA IF NOT EXISTS extensions;
  
  -- Move extensions that might still be in public
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'plpgsql' AND extnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
    ALTER EXTENSION plpgsql SET SCHEMA extensions;
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  -- Continue if extensions can't be moved (might already be in correct schema)
  NULL;
END $$;

-- 3. Create production monitoring tables
